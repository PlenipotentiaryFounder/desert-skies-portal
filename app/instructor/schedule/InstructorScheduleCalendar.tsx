...
export function InstructorScheduleCalendar({ sessions = [] }: { sessions: any[] }) {
  ...
  const events = sessions.map((s) => ({
    id: s.id,
    title: `${s.student ? s.student.first_name + " " + s.student.last_name : "Session"} (${s.session_type})`,
  ...
  async function handleCancel() {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Session canceled", description: "The student has been notified.", variant: "default" })
  ...
  async function handleReschedule(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Reschedule request sent", description: "The student will be notified.", variant: "default" })
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                <div className="space-y-2">
                  <div><b>Student:</b> {selected?.resource?.student ? `${selected.resource.student.first_name} ${selected.resource.student.last_name}` : selected?.resource?.enrollment_id}</div>
        ...
        <Dialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Are you sure you want to cancel this session? This cannot be undone. The student will be notified.</DialogDescription>
            </DialogHeader>
            <DialogFooter>
          ...
          <SheetContent side="right" className="max-w-md w-full">
            <form onSubmit={handleReschedule} className="space-y-4">
              ...
              <div>
                <label>Student</label>
                <Select
                  value={rescheduleForm?.enrollment_id || ""}
                  onValueChange={val => setRescheduleForm((f: any) => ({ ...f, enrollment_id: val }))}
                  disabled={optionsLoading}
                >
                  <SelectTrigger>
                    <SelectValue placeholder={optionsLoading ? "Loading..." : "Select a student"} />
                  </SelectTrigger>
                  <SelectContent>
