...
import { payFromPrepaidHours } from "@/lib/payment-service"
...
export async function POST(
  ...
  try {
    ...
    // Process payment from prepaid hours
    const result = await payFromPrepaidHours(
      params.id,
      invoice.student_id,
    ...
    return NextResponse.json(
      { 
        success: false,
        error: "Failed to process prepaid hours payment" 
      },
      { status: 500 }
    )
  }
