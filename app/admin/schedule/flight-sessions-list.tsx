...
import { Calendar, Clock, Edit, MoreHorizontal, Plane, Trash2, User, Plus } from "lucide-react"
...
import { DeleteFlightSessionDialog } from "./delete-flight-session-dialog"

interface FlightSessionsListProps {
  initialSessions: FlightSession[]
}

export function FlightSessionsList({ initialSessions }: FlightSessionsListProps) {
  const [sessions, setSessions] = useState<FlightSession[]>(initialSessions)
  const [searchTerm, setSearchTerm] = useState("")
  const [statusFilter, setStatusFilter] = useState<string>("all")
  const [showDeleteDialog, setShowDeleteDialog] = useState(false)
  const [sessionToDelete, setSessionToDelete] = useState<FlightSession | null>(null)
  ...
  const handleDeleteClick = (session: FlightSession) => {
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row">
        <div className="flex-1">
          <Input
            placeholder="Search by student, instructor, aircraft, or lesson..."
            value={searchTerm}
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-8 text-center">
          ...
          <p className="mb-4 mt-1 text-sm text-muted-foreground">
            {searchTerm || statusFilter !== "all"
              ...
              : "Get started by creating a new flight session"}
          </p>
          <Button asChild>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem asChild>
                            <Link href={`/admin/schedule/${session.id}`}>View Details</Link>
                          </DropdownMenuItem>
                          <DropdownMenuItem asChild>
