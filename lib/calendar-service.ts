...
// Google Calendar API methods (private helpers)
...
// Outlook Calendar API methods (private helpers)
...
export async function getCalendarEvents(
  ...
  switch (provider) {
    case 'google':
      return getGoogleCalendarEvents(accessToken, startTime, endTime, maxResults)
    case 'outlook':
      return getOutlookCalendarEvents(accessToken, startTime, endTime, maxResults)
    default:
...
export async function updateCalendarEvent(
  ...
  switch (provider) {
    case 'google':
      await updateGoogleCalendarEvent(accessToken, externalEventId, event)
      break
    case 'outlook':
      await updateOutlookCalendarEvent(accessToken, externalEventId, event)
      break
    default:
      throw new Error(`Unsupported provider: ${provider}`)
  }
...
export async function saveExternalCalendarEvent(
  userId: string,
  connectionId: string,
...
export async function deleteExternalCalendarEvent(externalEventId: string): Promise<void> {
  const cookieStore = await cookies()
  const supabase = await createClient(cookieStore)
