...
export class CalendarSyncService {
  ...
  static async syncUserCalendars(userId: string): Promise<SyncResult> {
    ...
    try {
      ...
      for (const connection of connections) {
        await CalendarService.logSyncOperation(
          connection.id,
          'refresh',
      ...
      for (const session of flightSessions) {
        try {
          ...
          if (existingExternalEvent) {
            ...
            const externalId = await CalendarService.createCalendarEvent(connectionId, calendarEvent)

            // Save the mapping in our database
            await CalendarService.saveExternalCalendarEvent(userId, connectionId, {
              ...calendarEvent,
  ...
  static async importExternalEventsToApp(
    ...
    try {
      ...
      const externalEvents = await CalendarService.getCalendarEvents(connectionId, startTime, endTime, 500)
      ...
      for (const event of nonFlightEvents) {
        try {
          ...
          if (!existingEvent) {
            ...
            await CalendarService.saveExternalCalendarEvent(userId, connectionId, event)
            result.records_processed++
          } else {
            // Update existing event if needed
      ...
      for (const connection of connections) {
        try {
          if (operation === 'delete') {
            ...
            if (existingEvent) {
              await CalendarService.deleteCalendarEvent(connection.id, existingEvent.external_id)
              await CalendarService.deleteExternalCalendarEvent(existingEvent.id)
            }
          } else {
  ...
  private static async updateExternalEventIfChanged(
    ...
    const hasChanged =
      ...
      existingEvent.end_time !== newEvent.end_time ||
      existingEvent.location !== newEvent.location ||
      existingEvent.status !== newEvent.status

    if (hasChanged) {
      await CalendarService.updateCalendarEvent(connectionId, existingEvent.external_id, newEvent)
    }
  }
